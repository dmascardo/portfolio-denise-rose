...
- ssh into the server
    - For Mac and Linux: `ssh root@dnsecode.site`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo dnsecode`
    - replace your-username with your own
- create password
    - `passwd dnsecode`
- Move .ssh key file
    - `cp -r .ssh /home/dnsecode`
- change ownership of the .ssh directory
    - `cd /home/dnsecode`
    - `chown -R dnsecode:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh dnsecode@dnsecode.site`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- create a directory for your deployment
    - `sudo mkdir /var/www/dnsecode.site/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/dnsecode.site/html`
...
- `sudo vim /etc/nginx/sites-available/dnsecode.site`
...
server {
    ...
    root /var/www/dnsecode.site/html;
    index index.html index.htm index.nginx-debian.html;

    server_name dnsecode.site www.dnsecode.site;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/dnsecode.site /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/dnsecode/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/dnsecode/.ssh/github_deploy_key.pub >> /home/dnsecode/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/dnsecode/.ssh/github_deploy_key`
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ...
         scp -r ./dist/* gkephart@dnsecode.site:/var/www/dnsecode.site/html
```
